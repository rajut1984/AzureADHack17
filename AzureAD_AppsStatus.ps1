...
function GetJobID
{
	Trace "Debug" "Inside GetJobID function"
    $nqext.GetJobID()
}
...
{
	Trace "Debug" "Inside IterationCount() function"
...
function DictionaryInsertValue
{
	...
	Trace "Debug" "Inside the function DictionaryInsertValue()"
...
function DictionaryRetrieveValue
{
	...
	Trace "Debug" "Inside the function DictionaryRetrieveValue()"
...
function GetDefaultResourceAndFullPath
{
	...
	else
	{
		...
		$global:azuread_groupsinfo    = $AzureAD_Users_Groups
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_groupsinfo =$global:azuread_groupsinfo"
        $global:azuread_groupsinfo_resource  = "AzureAD_Users_Groups"
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_groupsinfo_resource =$global:azuread_groupsinfo_resources"
		
		$global:azuread_domainstatus    = $AzureAD_Domains
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_domainstatus =$global:azuread_domainstatus"
        $global:azuread_domainstatus_resource  = "AzureAD_Domains"
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_domainstatus_resource =$global:azuread_domainstatus_resource"
		
		$global:azuread_appsstatus    = $AzureAD_Apps
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_appsstatus =$global:azuread_appsstatus"
        $global:azuread_appsstatus_resource  = "AzureAD_Apps"
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_appsstatus_resource =$global:azuread_appsstatus_resource"
	...
	if ($global:azuread_groupsinfo)
    {
        $target_fullpath = $global:azuread_groupsinfo
        $target_resource = "$global:azuread_groupsinfo_resource = $global:azuread_groupsinfo_resource"
    }
	
	if ($global:azuread_domainstatus)
    {
        $target_fullpath = $global:azuread_domainstatus
        $target_resource = "$global:azuread_domainstatus_resource = $global:azuread_domainstatus_resource"
    }
	
	if ($global:azuread_appsstatus)
    {
        $target_fullpath = $global:azuread_appsstatus
        $target_resource = "$global:azuread_appsstatus_resource = $global:azuread_appsstatus_resource"
    }
...
function GetMonitoredAzureADDomainNames
{
	...
	if ($global:azuread_appsstatus)
    {
        $monitored_azuread_domainnames = $global:azuread_appsstatus.Split(',')
...
$global:azuread_appsstatus    = ""
$global:exchangeonline_appsstatus_resource  = ""
...
TraceJobStart "AzureAD_AppsStatus"



if($global:azuread_appsstatus)
{
    Trace "Debug" "Monitoring Azure AD Apps Status and Health- Enter"
	...
	foreach ($domain in $monitored_azuread_domains)
	{
		...
		$azureAD_apps_count_TH=0
		$azureAD_new_apps=@()
		$azureAD_deleted_apps=@()
		...
		try
		{
			...
			if ($global:connect_success)
			{
				...
				if ( $PRM_Apps_Crossed_TH -eq $true)
				{
					$appscount = 0
					#$appscount = Get-AzureADDevice -All 1 | Measure
					$appscount = Get-AzureADApplication | select-object ObjectId,DisplayName
					$applicationcount = $appscount.count
					
					Trace "Debug" "Total applications count $applicationcount"
					
					if ($applicationcount -gt $PRM_Apps_Count_Threshold)
					{
					
						Trace "Debug" "Main: Registered Apps Count Crossed the threshold limit"
						$event_msg  = "Apps Count on AzureAD:$domain exceeded threshold."
						$detail_message = "Total # of Apps = $applicationcount ,Threshold Limit = $PRM_Apps_Count_Threshold `n"
						$detail_message += "`n Number of Apps on AzureAD:$domain crossed the set throughold `n"
						$detail_message += "Either change the limit or block Apps registration sync `n `n"
						$detail_message += "List of Registered Apps in Azure AD `n"
						$detail_message += "------------------------------------`n"
						
						
						foreach ($app in $appscount)
						{
							$appsName = $app.DisplayName
							$detail_message += " - $appsName `n"
						}
						
						
						CreateEvent $PRM_Apps_TH_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
						
					}
					else
					{
						Trace "Debug" "Main: Apps Count not Crossed the threshold limit"
					}
				} # Done till here # Raju
				
				if ( $PRM_No_Owner_Assigned_Apps_Status -eq $true)
				{
					$ownerAssigned = $false
										
					$apps_data = Get-AzureADApplication | select-object ObjectId,DisplayName

					$noUserApps = @()

					$appscount = $apps_data.count
					$i = 0
					
					if ($appscount.count -gt 0)
					{
						foreach ($app in $apps_data)
						{
						
							$appObjecID = $app.ObjectId
							$appName = $app.DisplayName
							
							Trace "Debug" " AppName - $appName , ObjId - $appObjecID" #$app.ObjectId
									
							$app_owner = Get-AzureADApplicationOwner -ObjectId $appObjecID 
							
							$AppOwner = $app_owner.DisplayName 
							Trace "Debug" " Assigned Owner of App  $appName is $AppOwner" #$app.ObjectId
							
									
							if( $AppOwner)
							{
								Trace "Debug" "$appName having owner assigned"
								
							}
							else
							{
								Trace "Debug" "No user/owner assigned to App $appName "
								$noUserApps += $appName;
								$list_noUserApps_message += " - $appName `n"
								$i += 1
								
							}
						
						}
						Trace "Debug" "List of Registered Apps with no Owners/Users assigned : $noUserApps"
						
						$event_msg  = "Registered Apps found with no Owner/User assigned"
						$detail_message = "Total # of Registered Apps 			     = $appscount `n" 
						$detail_message += "Total # of Registered Apps with no Owner/user = $i `n"
						$detail_message += "`nList of Apps with no Owner/User `n"
						$detail_message += "------------------------- `n"
						$detail_message += "$list_noUserApps_message "
						$detail_message += "------------------------- `n"
						
						CreateEvent $PRM_No_Owner_Assigned_Apps_Status_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
					}
					else
					{
						Trace "Error" "Main: Registered Apps are not found in azureAD account"
					}
				
				}
				
				if ( $PRM_Apps_New -eq $true)
				{
					
					
					$iteration = IterationCount
					
					Trace "Debug" "Iteration # : $iteration"
					
					$jobId = GetJobID
					
					Trace "Debug" "JobID # : $jobId"
					
					
					$Nqext_DicKey = "Apps" + ":" +(GetJobID) + ":" + "AppsNew" 
					
					Trace "Debug" "dict_key is: $Nqext_DicKey"
					...
					Trace "Debug" "Total # of Apps : $applicationcount"
					...
					if ($iteration -eq 1)
					{
						...
						Trace "Debug" "Interation # is $iteration"
						...
						Trace "Debug" "Dictionaly value to be inserted in iteration # 1 is  - $DicValue"
						
						DictionaryInsertValue $Nqext_DicKey $DicValue
					}
					else
					{
					
						$DicValueOld = DictionaryRetrieveValue $Nqext_DicKey
						
						Trace "Debug" "Existing Dictionaly value from previous iteration is - $DicValueOld"
						...
						if ($applicationcount -gt 0)
						{
							...
							{
								$appsName = $app.DisplayName
								$DicValueFresh += "$appsName" + ":"
							}

							$DicValueFresh = $DicValueFresh.Trim(":"," ")	
						}
						
						Trace "Debug" "Dictionaly value to be inserted in iteration # $iteration is  - $DicValueFresh"
						
						DictionaryInsertValue $Nqext_DicKey $DicValueFresh
						
						
						# Now compare the 2 arrays ( existing and new values) , to find if any new Apps got registered or not 
						
						$arrayOfOldApps = @();
						$arrayOfNewApps = @();
						
						$arrayOfOldApps = $DicValueOld.split(":")												
						$arrayOfNewApps = $DicValueFresh.split(":")
						
						Trace "Debug" "====================================================================================================="
						Trace "Debug" "List of Old Apps : $arrayOfOldApps"
						Trace "Debug" "====================================================================================================="
						Trace "Debug" "List of New Apps : $arrayOfNewApps"
						Trace "Debug" "====================================================================================================="

						$hash= @{}

						#storing elements of $arrayOfOldApps in hash
						foreach ($i in $arrayOfOldApps)
						{$hash.Add($i, "present")}
						...
						foreach($j in $arrayOfNewApps)
						{
							if(!$hash.ContainsKey($j))
							{
							    Trace "Debug" " The App $j doesnot exists in hash"
								$c = $c+$j
							}
							else 
							{
								Trace "Debug" " The App $j exists in hash"
								$hash.Remove($j)
							}
						}

						Trace "Debug" "New Apps found are : $c"
						
						$CCount = $c.Count
						
						Trace "Debug" "# of New Apps registered  : $CCount"

						
						# Getting deleted Apps info
						
						Trace "Debug" "Deleted Apps are : "
						...
						Trace "Debug" "# of Deleted Apps   : $DCount"

												
						if ($CCount -gt 0)
						{
							Trace "Debug" "Main: New Apps(s) got registered in Azure AD"
							$event_msg  = "New Apps(s) registered in AzureAD:$domain."
							$detail_message = "Total # of new Apps(s) registered in AzureAD:$domain is - $CCount`n`n"
							$detail_message += "Details of new App(s) `n"
							$detail_message += "--------------------`n"
							$detail_message += "$c `n"
							
							CreateEvent $PRM_Apps_New_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
				...
				if ( $PRM_Apps_Delete -eq $true)
				{
					...
					$Nqext_DicKey = "Apps" + ":" +(GetJobID) + ":" + "AppsDelete" 
					...
					else
					{
						...
						# Now compare the 2 arrays ( old and new values) , to find if any existing App(s) got deleted or not 
						...
						if ($DCount -gt 0)
						{
							Trace "Debug" "Main: Existing Registered Apps(s) got deleted in Azure AD"
							$event_msg  = "Apps(s) deleted in AzureAD:$domain."
							$detail_message = "Total # of deleted Apps(s) in AzureAD:$domain is - $DCount`n`n"
							$detail_message += "Details of deleted App(s)`n"
							$detail_message += "--------------------`n"
							foreach($k in $hash.keys)
							{
								#$c = $c + $k
							...
							CreateEvent $PRM_Apps_Delete_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
		...
		{		    
			Trace "Debug" "Main: Exception raised while attempting to fetch Azure AD Apps information for the domain $domain"
		 	$error_msg = $_.Exception.Message
			$event_msg = "Failed to retrieve Apps details for $domain"
	...
	Trace "Debug" "Monitoring Azure AD Apps Info - Exit"
}
TraceJobCompletion "AzureAD_AppsStatus"
