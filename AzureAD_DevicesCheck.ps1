...
function GetDefaultResourceAndFullPath
{
	...
	else
	{
		...
		$global:azuread_devicescheck    = $AzureAD_Devices
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_devicescheck =$global:azuread_devicescheck"
        $global:azuread_devicescheck_resource  = "AzureAD_Devices"
		Trace "Debug" "GetDefaultResourceAndFullPath: global:azuread_devicescheck_resource =$global:azuread_devicescheck_resource"
	...
	if ($global:azuread_devicescheck)
    {
        $target_fullpath = $global:azuread_devicescheck
        $target_resource = "$global:azuread_devicescheck_resource = $global:azuread_devicescheck_resource"
    }
...
function GetMonitoredAzureADDomainNames
{
	...
	if ($global:azuread_devicescheck)
    {
        $monitored_azuread_domainnames = $global:azuread_devicescheck.Split(',')
...
$global:azuread_devicescheck    = ""
$global:exchangeonline_devicestatus_resource  = ""
...
TraceJobStart "AzureAD_DevicesCheck"



if($global:azuread_devicescheck)
{
    Trace "Debug" "Monitoring Azure AD Device(s) State and Health- Enter"
	...
	foreach ($domain in $monitored_azuread_domains)
	{
		...
		$azureAD_devices_count_TH=0
		$azureAD_new_groups=@()
		$azureAD_deleted_groups=@()
		...
		try
		{
			...
			if ($global:connect_success)
			{
				...
				if ( $PRM_Devices_Crossed_TH -eq $true)
				{
					$devicecount = 0
					$devicecount = Get-AzureADDevice -All 1 | Measure
					$dvcount = $devicecount.count
					
					Trace "Debug" "Total devices count $dvcount"
					
					if ($dvcount -ge $PRM_Devices_Count_Threshold)
					{
						Trace "Debug" "Main: Devices Count Crossed the threshold limit"
						$event_msg  = "Devices Count on AzureAD:$domain exceeded threshold."
						$detail_message = "Total # of Devices = $dvcount ,Threshold Limit = $PRM_Devices_Count_Threshold `n`n"
						$detail_message += "Number of Devices on AzureAD:$domain crossed the set threshold `n"
						$detail_message += "Either change the limit or block Devices sync"
						
						CreateEvent $PRM_Devices_TH_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
						
					}
					else
					{
						Trace "Debug" "Main: Devices Count not Crossed the threshold limit"
					}
				}
				
				if ( $PRM_SysAD_Devices_Sync_Status -eq $true)
				{
					$syncStatus = $false
					$devices_DirSync_false = Get-azureADDevice | where-Object {$_.DeviceTrustType -eq 'ServerAd' -and $_.DirSyncEnabled -eq 1} | Select-Object DisplayName | Measure
					
					#$groupcount = Get-AzureADGroup -All 1 | Measure
					$devices_DirSync_false_count = $devices_DirSync_false.count
					
					Trace "Debug" "Total number of Un-Synced System Active Directory Device count = $devices_DirSync_false_count"
					
					if ($devices_DirSync_false_count -ge 0)
					{
						Trace "Debug" "Main: There are System AD devices with Directory Sync not enabled in your AzureAD account"
						$event_msg  = "System AD Devices Directory Sync Status disabled."
						$detail_message = "Total # of Devices with Directory Sync Status disabled = $devices_DirSync_false_count `n"
						$detail_message += "List of Dir Sync status disabled Devices `n"
						$detail_message += "------------------------- `n"
						
						
						$devices_DirSync_False = Get-azureADDevice | where-Object {$_.DeviceTrustType -eq 'ServerAd' -and $_.DirSyncEnabled -eq 1} | Select-Object DisplayName
						
						foreach ($dvname in $devices_DirSync_False)
						{
							$dvnameTemp = $dvname.DisplayName
							Trace "Debug" " $dvnameTemp"
							$detail_message += " - $dvnameTemp `n"
						}
						
						Trace "Debug" "Detailed Message --- $detail_message"
						#$detail_message += "$domains_not_verified "
						
						CreateEvent $PRM_SysAD_Devices_Sync_Status_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
						
					}
					else
					{
						Trace "Error" "Main: Directory Sync disabled devices not found in azureAD account"
					}
				}
				if ( $PRM_Devices_New -eq $true)
				{
					
					Trace "Debug" "Main: New Device added/synced in Azure AD"
					$event_msg  = "New Device(s) added in AzureAD:$domain."
					$detail_message = "New Device(s) added/synced in AzureAD:$domain `n"
					$detail_message += "Details"
						
					CreateEvent $PRM_Devices_New_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
					
				}
				if ( $PRM_Devices_Delete -eq $true)
				{
					
					Trace "Debug" "Main:  Existing Device(s) deleted in Azure AD"
					$event_msg  = "Existing Device(s) deleted in AzureAD:$domain."
					$detail_message = "Existing Device(s) deleted in AzureAD:$domain `n"
					$detail_message += "Details"
						
					CreateEvent $PRM_Devices_Delete_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
		...
		{		    
			Trace "Debug" "Main: Exception raised while attempting to fetch Azure AD devices information for the domain $domain"
		 	$error_msg = $_.Exception.Message
			$event_msg = "Failed to retrieve device details for $domain"
	...
	Trace "Debug" "Monitoring Azure AD Devices Info - Exit"
}
TraceJobCompletion "AzureAD_DevicesCheck"
