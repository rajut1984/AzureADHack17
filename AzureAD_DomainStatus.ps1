...
function GetMonitoredAzureADDomainNames
{
	...
	if ($global:azuread_groupsinfo)
    {
        $monitored_azuread_domainnames = $global:azuread_groupsinfo.Split(',')
	...
	if ($global:azuread_domainstatus)
    {
        $monitored_azuread_domainnames = $global:azuread_domainstatus.Split(',')
        $monitored_azuread_domainnames = $monitored_azuread_domainnames | ForEach-Object { $_.Split(':')[-3] }
...
$global:azuread_domainstatus    = ""
$global:exchangeonline_domainstatus_resource  = ""
...
TraceJobStart "AzureAD_DomainStatus"



if($global:azuread_domainstatus)
{
    Trace "Debug" "Monitoring Azure AD Domains(s) Status and Health- Enter"
	...
	foreach ($domain in $monitored_azuread_domains)
	{
		...
		try
		{
			...
			if ($global:connect_success)
			{
				...
				if ( $PRM_Domains_Status_Unverify -eq $true)
				{
					$unverified = $false
					$domains_not_verified = Get-AzureADDomain | Where-Object {$_.IsVerified -ne "True"} | Select-Object Name 
					#$groupcount = Get-AzureADGroup -All 1 | Measure
					$not_verified_domain_count = $domains_not_verified.count
					
					Trace "Debug" "Total number of  UnVerified domains count = $not_verified_domain_count"
					
					if ($not_verified_domain_count -ge 0)
					{
						Trace "Debug" "Main: There are unverified domins in your AzureAD account"
						$event_msg  = "Domain(s) status is UnVerifed."
						$detail_message = "Total # of UnVerifed domains = $not_verified_domain_count `n`n"
						$detail_message += "List of UnVerified Domains `n"
						$detail_message += "------------------------- `n"
						
						foreach ($domname in $domains_not_verified)
						{
							$domnameTemp = $domname.Name
							Trace "Debug" " $domnameTemp"
							$detail_message += " - $domnameTemp `n"
						}
						
						Trace "Debug" "Detailed Message --- $detail_message"
						#$detail_message += "$domains_not_verified "
						
						CreateEvent $PRM_Domains_Status_Unverify_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
						
					}
					else
					{
						Trace "Error" "Main: UnVerified domains not found in azureAD account"
					}
				}
				if ( $PRM_CusomtomDomain_New -eq $true)
				{
					
					Trace "Debug" "Main: New Domain Created in Azure AD"
					$event_msg  = "New Domain(s) created in AzureAD:$domain."
					$detail_message = "New Domain(s) got created in AzureAD:$domain `n"
					$detail_message += "Details"
						
					CreateEvent $PRM_Group_New_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
					
				}
				if ( $PRM_CusomtomDomain_Deleted -eq $true)
				{
					
					Trace "Debug" "Main:  Existing Domain(s) deleted in Azure AD"
					$event_msg  = "Existing Domain(s) deleted in AzureAD:$domain."
					$detail_message = "Existing Domain(s) deleted in AzureAD:$domain `n"
					$detail_message += "Details"
						
					CreateEvent $PRM_Group_Deleted_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
		...
		{		    
			...
			$event_msg = "Failed to retrieve group details for $domain"
	...
	Trace "Debug" "Monitoring Azure AD Group Info - Exit"
}
TraceJobCompletion "AzureAD_GroupsInfo"
