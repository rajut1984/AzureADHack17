...
$global:azuread_groupsinfo    = ""
$global:exchangeonline_groupsinfo_resource  = ""
...
TraceJobStart "AzureAD_GroupsInfo"



if($global:azuread_groupsinfo)
{
    Trace "Debug" "Monitoring Azure AD Group's Activities - Enter"
	...
	foreach ($domain in $monitored_azuread_domains)
	{
		...
		try
		{
			...
			if ($global:connect_success)
			{
				...
				if ( $PRM_Groups_Crossed_TH -eq $true)
				{
					$groupcount = 0
					$groupcount = Get-AzureADGroup -All 1 | Measure
					$gpcount = $groupcount.count
					
					Trace "Debug" "Total group count $gpcount"
					
					if ($gpcount -ge $PRM_Groups_Count_Threshold)
					{
						Trace "Debug" "Main: Groups Count Crossed the threshold limit"
						$event_msg  = "Groups Count on AzureAD:$domain exceeded threshold."
						$detail_message = "Total # of Users = $usrcount ,Threshold Limit = $PRM_Users_Count_Threshold `n`n"
						$detail_message += "Number Groups on Azure AD for $domain has crossed the set throughold `n"
						$detail_message += "Either change the limit or block group creation"
						
						CreateEvent $PRM_Groups_TH_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
						#
					}
					else
					{
						Trace "Error" "Main: Groups Count not Crossed the threshold limit"
					}
				}
				if ( $PRM_New_Group_Creat -eq $true)
				{
					
					Trace "Debug" "Main: New Group Created in Azure AD"
					$event_msg  = "New Group created in Azure AD."
					$detail_message = "New Group got created in Azure AD `n"
					$detail_message += "Details"
						
					CreateEvent $PRM_Groups_TH_EventSeverity $event_msg $detail_message $Event_target $action_to_be_taken
					
				}
				if ( $PRM_Group_Deleted -eq $true)
				{
					Trace "Debug" "Main:  Group deleted in Azure AD"
					$event_msg  = "Group deleted in Azure AD."
					$detail_message = " Group got detelted in Azure AD `n"
					$detail_message += "Details"
