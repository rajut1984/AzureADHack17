...
#                                    
$global:script_version = '7.6.109.0' 
...
function GetSecurityContext
{
	...
	Trace "Debug" "Security Context: Lable - $lable , Sublabel - $sublabel "
	...
	Trace "Debug" "End of GetSecurityContext function:   $result"
    $result
}
...
function GetCredentials
{
    ...
    $credentials = $null
	 ...
	 Trace "Debug" "Inside GetCredentials seems old function"
...
function GetDefaultResourceAndFullPath
{
	...
	# Raju : Check how to add Azure AD info here 
...
#
function GetMonitoredExchangeOnlineDomainNames
{
    $monitored_exchangeOnline_domainnames = $null
    ...
    return $monitored_exchangeOnline_domainnames
}
...
#       the object path.
#
# Raju : Azure AD should come here
...
function Get-DiscoveryObjectTypes([System.String]$component, [System.String]$types)
{
    "<OBJTYPE COMPONENT_NAME='$component'>`n$types</OBJTYPE>"
}


function Get-DiscoveryObjectType([System.String]$name, [System.Int32]$icon_id, [String[]]$detail_header)
{
    $xml = "<$Name OBJ_TYPE_NAME='$name' IconID='$icon_id'"
    for ($i = 1; $i -le $detail_header.Count; $i++) {
        $header = $detail_header[$i-1]
        if ($header.Length -eq 0) {
            throw "ERROR: HEADER$i is empty"
        }
        $xml += " HEADER$i='$header'"
    }
    $xml + "/>"
...
function Get-DiscoveryObjects([System.String]$view, [System.String]$objects)
{
    "<OBJECT VIEW_NAME='$view'>`n$objects</OBJECT>"
}


function Get-DiscoveryObject([System.String]$type, [System.String]$name, [String[]]$detail, [System.String]$elements = "")
{
    $name = $name.Replace("'","&apos;")
    $name = $name.Replace('"',"&quot;")

    $xml_inst = "<$type INST_NAME='$name'"

    for ($i = 1; $i -le $detail.Count; $i++)
    {
        $temp_detail = $($detail[$i-1])

        if($temp_detail)
        {
            $temp_detail = $temp_detail.Replace("'","&apos;")
            $temp_detail = $temp_detail.Replace('"',"&quot;")
        }

        $xml_inst += " DETAIL$i='$temp_detail'"
    }

    if ($elements.Length -gt 0) {
        $xml_inst += ">`n$elements</$Type>`n"
    }
    else {
        $xml_inst += "/>`n"
    }
...
function Get-DiscoveryAzureADObjectTypes
{
	$azuread_header          = "Identity",    "Domain"
	$azuread_connect_header          =  "Identity",    "Domain"
	$azuread_domains_detail_header     =   "Identity",    "Domain"
	$azuread_users_groups_detail_header     =   "Identity",    "Domain"
	$azuread_licenses_header      =   "Identity",    "Domain"
	$azuread_apps_header      =   "Identity",    "Domain"
	$azuread_devices_header      =   "Identity",    "Domain"
	
	$azuread_domain_name =  "Identity",	"Domain Controller"
	$azuread_license_name =  "Identity",	"SKU Type"
	$azuread_device_name = 	"Identity",		"Device Type"
	

    $object_type_data = `
		("AzureAD",                     29, $azuread_header), `
		("AzureAD_Connect",                     63, $azuread_connect_header), `
		("AzureAD_Domains",              187, $azuread_domains_detail_header),  `
		("AzureAD_Users_Groups",              249,  $azuread_users_groups_detail_header),  `
		("AzureAD_Licenses",              225,  $azuread_licenses_header),  `
		("AzureAD_Apps",              219,  $azuread_apps_header),  `
		("AzureAD_Devices",              13,  $azuread_devices_header),  ` 
		("AzureAD_DomainName",              187,  $azuread_domain_name),  ` 
		("AzureAD_LicenseName",              225,  $azuread_license_name),  ` 
		("AzureAD_DeviceName",              13,  $azuread_device_name)  ` 
		
    $types = ""
    foreach ($t in $object_type_data) {
        $types += Get-DiscoveryObjectType $t[0] $t[1] $t[2] $t[3] $t[4]
        $types += "`n"
    }

    Get-DiscoveryObjectTypes -Component "AzureAD" $types
}
...
function ConnectAzureAD([System.String]$val1, [System.String]$val2, [System.String]$val3)
{
	...
	#----------------Office365---------------------------
    #
	Trace "Debug" "Initializing Connection to AzureAD..."
...
Trace "Debug" "Main: Starting discovery powershell script execution ..."

$action_to_be_taken   = $AKPID
$job_failure_severity = $PRM_SeverityFail
$global:detail_message = ""
$global:connect_success = 0
$domain_count = 0
$no_of_domains_discovered = 0
$event_message_prefix = "Azure AD Discovery "
$domains_discoverySuccess=""
$domains_discoveryFailed=""
$global:Domain_Discovery_Details= @()
$server_name = [Environment]::MachineName
$Vals = @()
$Incorrect_SM_msg = "NOTE: When entering the credentials in Security Manager you must check the`n" `
                    ...
                    + "`tTo specify the credentials, create an entry in Security Manager as follows`n" `
                    + "`n" `
                    + "`tLabel     :     AzureAD`n" `
                    + "`tSub-Label :     <Domain>`n" `
                    + "`tValue 1   :     <Username>`n" `
...
Trace "Debug" "Main: on host $server_name...resource_and_fullpath = $resource_and_fullpath"
Trace "Debug" "Main: Starting Azure AD discovery powershell script execution on host $server_name..."

Trace "Debug" "Main: Discovering AzureAD ..."
$domainNames = $PRM_Domains -split ','| Where { $_ -match '\S' }
$domain_count = $domainNames.count

$error.clear()
try
{
    Trace "Debug" "Main: Importing AzureAD module..`n"
	Import-Module AzureAD
	if ($error.Count -gt 0)
	{
		Trace "Debug" "Main: Error occurred while importing AzureAD module.."
        $event_msg = "$event_message_prefix failed"
		$global:detail_message = $error
		CreateEvent $PRM_SeverityDiscoveryFailed $event_msg $global:detail_message $NT_MachineFolder `
                    $PRM_ActionDiscoveryFailed
		exit 0
	}
...
{
       Trace "Debug" "Main: Exception happened while importing MSonline module.."
       $event_msg = "$event_message_prefix failed"
	   $global:detail_message = $_
	   CreateEvent $PRM_SeverityDiscoveryFailed $event_msg $global:detail_message $NT_MachineFolder `
                    $PRM_ActionDiscoveryFailed
	   exit 0
...
{
    Trace "Debug" "Getting security context for domain $domainName"
	$domainName=$domainName.trim()
	#$Vals = GetSecurityContext 'AzureAD' $domainName $true
	[string[]] $result = "pralay.roy","Control@123",""
	
	$Vals = $result
	$vals = @()
	$Vals = @("pralay.roy","Control@123"," ")
	
	Trace "Debug" "Getting security context for domain $domainName , $Vals[0] , $Vals[1] , $Vals[2]"
     if((!$Vals) -or (($Vals[0] -eq "") -or ($Vals[1] -eq "") -or ($Vals[2] -eq "")))
	{
		...
		$Object = New-Object PSObject
        $Object | add-member Noteproperty Domain       $($domainName +"  ")
	    $Object | add-member Noteproperty Status       "Failed   "		
        $Object | add-member Noteproperty "Reason For Failure"       "Missing or Incorrect Security Manager credentials for this domain`n"
			   
		$global:Domain_Discovery_Details+=$Object
		remove-variable Object
		$domains_discoveryFailed += $domainName +"`n"	
		Continue
	}
	...
	if ($global:connect_success)
	{
		...
		$instnm = "AzureAD:" + $domainName.ToUpper()
		
		
		#----------------------Constructing Discovery XML--------------------

		$objtype_xml = Get-DiscoveryAzureADObjectTypes
	
		$top_level_name = $instnm

		#$TenantID  = (Invoke-WebRequest https://login.windows.net/$domainName/.well-known/openid-configuration|ConvertFrom-Json).token_endpoint.Split('/')[3]
		$Tenantdetail = "Azure AD",$domainName                  
		$AzureAD_object = ""
	
	
	    $AzureAD_Connect_xml = Get-DiscoveryObject "AzureAD_Connect" "AD Connect" $Tenantdetail
		
		
		#$AzureAD_Domains_xml = Get-DiscoveryObject "AzureAD_Domains" "Domains" $Tenantdetail		
		#####################################################################################		
		# Discovery xml preperation for AzureAD  Domains
		
		$AzureAD_Domains_xml = "<AzureAD_Domains INST_NAME='Domains' DETAIL1='AzureAD' DETAIL2='$domainName'> `n"
		$xmlDomains = Get-AzureADDomain | Select-Object Name 
					
		$xml_domain_count= $xmlDomains.count
		
		Trace "Debug" "domains count : $xml_domain_count"
		
		foreach ($xmldomain in $xmlDomains)
		{
			$dname = $xmldomain.Name
			Trace "Debug" "In For Loop domains name  : $dname"
		   $AzureAD_Domains_xml += "<AzureAD_DomainName INST_NAME='$dname' DETAIL1='AzureAD' DETAIL2='Domain Controller'/> `n"
		}
				
		$AzureAD_Domains_xml += "</AzureAD_Domains>`n"
		#####################################################################################
		
		
		
		$AzureAD_Users_Groups_xml = Get-DiscoveryObject "AzureAD_Users_Groups" "Users Groups" $Tenantdetail
		
		
		#$AzureAD_Licenses_xml = Get-DiscoveryObject "AzureAD_Licenses" "Licenses" $Tenantdetail
		#####################################################################################		
		# Discovery xml preperation for AzureAD  Licenses
		
		$AzureAD_Licenses_xml = "<AzureAD_Licenses INST_NAME='Licenses' DETAIL1='AzureAD' DETAIL2='$domainName'> `n"
		
		$xmlLicenses = Get-AzureADSubscribedSku | Select-Object SkuPartNumber 
					
		$xml_licenses_count= $xmlLicenses.count
		
		Trace "Debug" "Licenses count : $xml_licenses_count"
		
		foreach ($xmllicense in $xmlLicenses)
		{
			$licname = $xmllicense.SkuPartNumber
			Trace "Debug" "In For Loop Licenses name  : $licname"
		   $AzureAD_Licenses_xml += "<AzureAD_LicenseName INST_NAME='$licname' DETAIL1='AzureAD' DETAIL2='Subscription'/> `n"
		}
				
		$AzureAD_Licenses_xml += "</AzureAD_Licenses>`n"
		#####################################################################################
		
		$AzureAD_Apps_xml = Get-DiscoveryObject "AzureAD_Apps" "Apps" $Tenantdetail
		
		
		#$AzureAD_Devices_xml = Get-DiscoveryObject "AzureAD_Devices" "Devices" $Tenantdetail
		#####################################################################################		
		# Discovery xml preperation for AzureAD  Domains
		
		$AzureAD_Devices_xml = "<AzureAD_Devices INST_NAME='Devices' DETAIL1='AzureAD' DETAIL2='$domainName'> `n"
		$xmlDevices = Get-azureADDevice | Select-Object DisplayName 
					
		$xml_device_count= $xmlDevices.count
		
		Trace "Debug" "devices count : $xml_device_count"
		
		foreach ($xmldevice in $xmlDevices)
		{
			$dvcname = $xmldevice.DisplayName
			Trace "Debug" "In For Loop device name  : $dvcname"
		   $AzureAD_Devices_xml += "<AzureAD_DeviceName INST_NAME='$dvcname' DETAIL1='AzureAD' DETAIL2='Domain Controller'/> `n"
		}
				
		$AzureAD_Devices_xml += "</AzureAD_Devices>`n"
		#####################################################################################
		
		
		$AzureAD_object += $AzureAD_Connect_xml
		$AzureAD_object += $AzureAD_Domains_xml
		$AzureAD_object += $AzureAD_Users_Groups_xml
		$AzureAD_object += $AzureAD_Licenses_xml
		$AzureAD_object += $AzureAD_Apps_xml
		$AzureAD_object += $AzureAD_Devices_xml
		
		$AzureAD_details =  "AzureAD",$domainName
		
		$AzureAD_xml = Get-DiscoveryObject "AzureAD" $top_level_name $AzureAD_details $AzureAD_object
		Trace "Debug" "AzureAD_xml  = $AzureAD_xml "

		#
		# Discovery XML
		#
		$xml  = "<?xml version='1.0'?>`n<APPMANAGER>`n"
		$xml += "$objtype_xml`n"
		$xml += Get-DiscoveryObjects -View "AzureAD" $AzureAD_xml 
		$xml += "`n</APPMANAGER>`n"

		CreateEvent $PRM_SeverityDiscoverySuccessful $script_version $xml "AzureAD" "AKP_DISCOVERY"			
		$domains_discoverySuccess += $domainName +"`n"
		
		$Object = New-Object PSObject
        $Object | add-member Noteproperty Domain       $($domainName +"  ")
	    $Object | add-member Noteproperty Status       "Success   "		
        $Object | add-member Noteproperty "Reason For Failure"      "-`n"
			   
		$global:Domain_Discovery_Details+=$Object
		remove-variable Object
		$no_of_domains_discovered = $no_of_domains_discovered + 1
	}
	else
	{	
...
Trace "Debug" "Main: Discovery work done - now raising events, if necessary"

# Create event
if ($no_of_domains_discovered -eq $domain_count) 
{
    if ($PRM_EventDiscoverySuccessful) 
	{
        $event_msg = "$event_message_prefix successful"
		$global:detail_message = "$($global:Domain_Discovery_Details | Format-Table -Autosize -Wrap |Out-String) `n`n"
        CreateEvent $PRM_SeverityDiscoverySuccessful $event_msg $global:detail_message $NT_MachineFolder `
                    $PRM_ActionDiscoverySuccessful       
    }
}
else
{
    # Sending failure event If exchange online connection either failed for all or some of the domains specified in the list of domains to discover		
	if($PRM_EventDiscoveryFailed)
	{
		$event_msg = "$event_message_prefix failed"
		$global:detail_message = "$($global:Domain_Discovery_Details | Format-Table -Autosize -Wrap |Out-String) `n`n"
		$global:detail_message+=$Incorrect_SM_msg
		if($domains_discoverySuccess -and $domains_discoveryFailed)
		{
		    $event_msg = "Failed to connect to Exchange Online for some of the domains"
		}		
        CreateEvent $PRM_SeverityDiscoveryFailed $event_msg $global:detail_message $NT_MachineFolder `
                    $PRM_ActionDiscoveryFailed
	}        
...
Trace "Debug" "Main: Exit ExchangeOnline discovery job ..."

# SIG # Begin signature block
# MIIXmwYJKoZIhvcNAQcCoIIXjDCCF4gCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU2yGRUDzsToZvKpJrVLzNrSVb
# kuigghLJMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQDPNr1Nplx
# uad2Hjv0veEvEoyjXzANBgkqhkiG9w0BAQEFAASCAQANckvvKdDNa/wtXNhZrFT3
# z+1WqCV5xS4ZvSl8u4U+N1+dHbXnCTbZmT0TrpZFOyPwAnR304Q5sasCTmAfmaMi
# qgzHvfp1N24lmdo7R3IRgCR0LeYfX8gimj6LNhOEmB8zylpMQJYJAgQE0DEe6Sbm
# IjkWwXFJxWCZQyNd+S6EGmxRh1eJdmIM5G+kAh4TnOXescKfNNdb6enmAlSmEfnQ
# dTYkBYxpF5ppHdTCo5w166ymDwmnT0UD4aIoL/iRshU49dfU+Y53TOTualoGk3bN
# 1Rvd0K8Rj4CKJpV0fErwTlhKNeaeinW728PepXV1GNuhepSJhT+T/OtvPwHkfVy3
...
# hvcNAQkFMQ8XDTE3MDQxNDA2NTEzMlowIwYJKoZIhvcNAQkEMRYEFNUANo3Fh1Ho
# jSpMJ4+GwksD2rBhMA0GCSqGSIb3DQEBAQUABIIBABB0dIGR/soyZoRU1PdMP4wj
# hQxOriuOo5/0db8jBcqs++fYdEO3NLQ5njZJbUN0LcyN+aNhHbNQtglwsEhifihv
# aeWOSNu9Y9wQHqRS09Sni4J/2cc7/C1N/G7+SshquU6j36sB135J8eT7/B75npFR
# +UIrXPUuFsjKUG+bBHzDF3CUkWKD8MjhkMhgx7nO74KWhhvMb8iLQdge8izaJvdG
# 3a2/QlQzP5zW4+XgfwenFqTYMYv4PX9yvIrzaG0KaSgJMeBmU6VZw0MLtcirKba+
# XZwF0zJZYru0vIiU5mF94C9IlYf9J9FrzPR/CbZpotU6nzCc+omXIOD4aiwKpe4=
